...
$nama_divisi = $_SESSION['user_name'];

// Ambil semua disposisi untuk user login
$query = "
    SELECT d.*, s.no_surat, s.tanggal, s.pengirim, s.perihal
    FROM tb_disposisi d
...
$queryStatusBaca = "
    SELECT 
        SUM(status_baca = 'Sudah Dibaca') AS sudah, 
        SUM(status_baca = 'Belum Dibaca') AS belum 
    FROM tb_disposisi 
    WHERE id_penerima = ?
";
$stmtBaca = $conn->prepare($queryStatusBaca);
$stmtBaca->bind_param("i", $id_login);
$stmtBaca->execute();
$dataBaca = $stmtBaca->get_result()->fetch_assoc();
$sudahDibaca = $dataBaca['sudah'] ?? 0;
$belumDibaca = $dataBaca['belum'] ?? 0;

// Hitung jumlah disposisi per bulan (Jan - Jun)
$jumlahPerBulan = [];
for ($bulan = 1; $bulan <= 6; $bulan++) {
  $stmtBulan = $conn->prepare("SELECT COUNT(*) AS jumlah FROM tb_disposisi d JOIN tb_suratmasuk s ON d.id_surat = s.id WHERE d.id_penerima = ? AND MONTH(s.tanggal) = ?");
  if (!$stmtBulan) {
    die("Prepare query bulan gagal: " . $conn->error);
  }
  $stmtBulan->bind_param("ii", $id_login, $bulan);
  $stmtBulan->execute();
  $hasil = $stmtBulan->get_result()->fetch_assoc();
  $jumlahPerBulan[] = $hasil['jumlah'] ?? 0;
}
...
<head>
  ...
  <style>
    ...
    .btn-aksi {
      ...
      cursor: pointer;
      transition: background-color 0.3s ease;
      box-shadow: 0 3px 8px rgba(156, 58, 58, 0.6);
    }

    .btn-aksi:hover {
      background-color: #c65353;
      box-shadow: 0 4px 12px rgba(197, 83, 83, 0.8);
    }
...
<body>
  ...
  <header class="navbar">
    ...
    <h1>Selamat Datang, <?= $nama_divisi ?></h1>
          ...
          <?php if ($result->num_rows > 0): ?>
            ...
            while ($row = $result->fetch_assoc()): ?>
              <tr>
                ...
                <td>
                  <a href="lihat_surat.php?id=<?= $row['id_disposisi'] ?>" class="file-link" target="_blank">Lihat</a>
                </td>
              </tr>
            <?php endwhile; ?>
          <?php else: ?>
  ...
  <script>
    ...
    window.onload = function () {
      ...
      if (belumDibaca > 0) {
        ...
        // Atau sembunyikan otomatis setelah 5 detik
        setTimeout(() => {
          notif.style.display = 'none';
        }, 5000);
      }
